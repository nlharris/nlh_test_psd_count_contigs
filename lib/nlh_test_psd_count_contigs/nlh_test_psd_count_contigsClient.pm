package nlh_test_psd_count_contigs::nlh_test_psd_count_contigsClient;
...
nlh_test_psd_count_contigs::nlh_test_psd_count_contigsClient
...
sub new
{
    ...
    my $self = {
	client => nlh_test_psd_count_contigs::nlh_test_psd_count_contigsClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a nlh_test_psd_count_contigs.workspace_name
$contigset_id is a nlh_test_psd_count_contigs.contigset_id
$return is a nlh_test_psd_count_contigs.CountContigsResults
...
# Authentication: required
	...
	method => "nlh_test_psd_count_contigs.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "nlh_test_psd_count_contigs.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for nlh_test_psd_count_contigs::nlh_test_psd_count_contigsClient\n";
    }
    if ($sMajor == 0) {
        warn "nlh_test_psd_count_contigs::nlh_test_psd_count_contigsClient version is $svr_version. API subject to change.\n";
    }
}
...
package nlh_test_psd_count_contigs::nlh_test_psd_count_contigsClient::RpcClient;
